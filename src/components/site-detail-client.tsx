...
export function SiteDetailClient({ site, forecast, scores }: SiteDetailClientProps) {
  ...
  // Get today's score for the flyability badge
  const todayScore = scores[0];

  return (
    <div className="space-y-6">
      {/* Flyability badge and summary */}
      {todayScore && (
        <Card className="p-4">
          <div className="flex items-center gap-4 flex-wrap">
            <Badge
              className={`${getScoreColorClass(todayScore.overallScore)} text-white text-base px-3 py-1.5`}
            >
              {todayScore.label}
            </Badge>
            <p className="text-sm text-muted-foreground flex-1">
              {todayScore.label === 'Epic' && 'Exceptional flying conditions expected today.'}
              {todayScore.label === 'Great' && 'Excellent flying conditions expected today.'}
              {todayScore.label === 'Good' && 'Good flying conditions expected today.'}
              {todayScore.label === 'Fair' &&
                'Marginal flying conditions. Check details carefully.'}
              {todayScore.label === 'Poor' && 'Poor flying conditions. Not recommended today.'}
            </p>
          </div>
        </Card>
      )}

      {/* Windgram and Table View tabs */}
